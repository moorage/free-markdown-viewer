...
- Open a local workspace root directory.
- Show a sidebar/drawer with the subdirectory tree and Markdown files.
- Render Markdown files natively using AppKit/TextKit 2 plus custom block renderers.
...
The app is a native document browser with four major subsystems:
...
The renderer must remain isolated from workspace browsing and navigation state.
...
AppCoordinator
    ...
    └─ TopBarViewController
            ↓
Markdown parser → AST → Render IR → Block layout → Native rendering
                                      ├─ Text blocks via TextKit 2
...
- Language: Swift
- UI: AppKit-first
- Text layout: TextKit 2
- Workspace tree: `NSOutlineView`
- Top-level layout: `NSSplitViewController`
...
This project should be built as a real macOS app target with XCTest/XCUITest, AppKit UI, assets, and command-line build/test control through `xcodebuild`. Apple documents Xcode as the full app-development suite and `xcodebuild` as the CLI entrypoint for Xcode projects/workspaces and schemes. The Command Line Tools package is useful for UNIX-style development, but for a normal macOS app target and UI automation, full Xcode is the correct setup. ([developers.openai.com](https://developers.openai.com/cookbook/examples/gpt-5/codex_prompting_guide/?utm_source=chatgpt.com))
...
- create an empty native macOS AppKit app project in Xcode, or
...
- XCUITest UI test target
- one shared scheme checked in

This removes a class of bootstrap risk.

### 5. Ensure UI testing permissions can work

The machine must be able to run UI tests locally. The human should be ready to grant any required local macOS permissions for test automation, screen recording, or accessibility if prompted by the OS.
...
Every important UI surface must have stable identifiers, including:
...
```json
{
  "workspaceRoot": "/repo/Fixtures/docs",
  "selectedFile": "/repo/Fixtures/docs/guide/intro.md",
  "history": {
    "backCount": 2,
    "forwardCount": 1
  },
...
- parser feature support
- media classification
- relative path resolution
- anchor resolution
- history stack behavior
- table width and row-height calculations
- syntax highlight tokenization cache behavior
...
- AST to render-model conversion
- link routing internal vs external
- workspace tree building
- navigation + scroll restoration logic
- visible-block computation
- image/video classification with local files
- debug JSON export correctness
...
- window resize relayout correctness
...
- app launches to a placeholder window
...
- `scripts/build`
- `scripts/test-unit`
- `scripts/test-ui` smoke launch

### Milestone 1: workspace browser and shell navigation scaffold

Deliverables:

- `WorkspaceController`
- sidebar using `NSOutlineView`
- open workspace root
- current file loading from sidebar selection
- top bar with back/forward disabled state

Verification:

- unit tests for workspace scan and filtering
- integration tests for tree model
- UI test selecting a file from sidebar
...
- UI tests for multi-file navigation and back/forward restore
